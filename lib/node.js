...
app.get('/indices/:luck', (req, res) => {
    const { luck } = req.params;
...
app.get('/trend/:type', (req, res) => {
    const { type } = req.params;
    axios
        .get('http://www1.nseindia.com/live_market/dynaContent/live_analysis/' + type.toLowerCase() + 's/nifty' + type + 's1.json')
        .then(answer => {
            res.status(200).header(header).send(answer.data);
        })
        .catch(error => {
