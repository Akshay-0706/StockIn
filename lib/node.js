...
app.get('/stock/:code/:range/:interval', (req, res) => {
    ...
    axios
        ...
        .catch(error => {
            // console.log("Request failed with status code 422");
            // console.log("Try changing range")
...
app.get('/indices', (req, res) => {
    rp('http://www.nseindia.com/api/allIndices')
        .then(answer => {
...
// app.get('/indices', (req, res) => {
//     axios
//         .get('http://www.nseindia.com/api/allIndices')
//         .then(answer => {
//             res.status(200).header(header).send(answer.data);
//         })
//         .catch(error => {
...
app.get('/jwt/sign/:uid/:email', (req, res) => {
    ...
    let token = jwt.sign({ uid: uid, email: email, }, "token", { expiresIn: 120 })
...
app.get('/trend/:trend', (req, res) => {
    ...
    rp('http://www1.nseindia.com/live_market/dynaContent/live_analysis/' + trend.toLowerCase() + 's/nifty' + trend + 's1.json')
        .then(answer => {
            res.status(200).header(header).send(answer.data);
        })
        .catch(error => {
...
//     const { trend } = req.params;
//     axios
//         .get('http://www1.nseindia.com/live_market/dynaContent/live_analysis/' + trend.toLowerCase() + 's/nifty' + trend + 's1.json')
//         .then(answer => {
//             res.status(200).header(header).send(answer.data);
//         })
//         .catch(error => {
...
app.get('/topindices', (req, res) => {
    axios
        ...
        .catch(error => {
            console.log("Unable to fetch Nifty data\n");
            res.status(200).header(header).send({});
        });
...
//             if (error) {
//                 console.log("Unable to fetch indices: " + error);
//                 res.status(200).header(header).send({});
//             } else {
